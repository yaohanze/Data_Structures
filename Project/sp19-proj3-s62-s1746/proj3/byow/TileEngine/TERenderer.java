...
public class TERenderer {
    ...
    public void renderFrame(TETile[][] world) {
        ...
        int j = (int) y - 2;
        String str = "";
        if (i >= 80 || i < 0 || j >= 30 || j < 0) {
            str = "nothing";
        } else {
            str = world[i][j].description();
        }
        StdDraw.setPenColor(Color.WHITE);
        StdDraw.text(str.length() / 2, 33, str);
        StdDraw.text(80 - avatarName.length() / 2, 33, avatarName);
        StdDraw.text(width / 2, 33, "Level: " + level);
        String currPoint = "Your point: " + point;
        String currTarget = "Target point: " + target;
        StdDraw.text(currPoint.length() / 2, 1, currPoint);
        StdDraw.text(80 - currTarget.length() / 2, 1, currTarget);
        StdDraw.show();
    }
